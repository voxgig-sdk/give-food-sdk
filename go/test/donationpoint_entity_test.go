...
func TestDonationpointEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		setup := donationpointBasicSetup(nil)
		...
		for _, _op := range []string{"list", "load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "donationpoint." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GIVE_FOOD_TEST_DONATIONPOINT_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		donationpointRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.donationpoint", setup.data)))
...
func donationpointBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "donationpoint", "DonationpointTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read donationpoint test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse donationpoint test data: " + err.Error())
	}
	...
	idmap := vs.Transform(
		[]any{"donationpoint01", "donationpoint02", "donationpoint03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GIVE_FOOD_TEST_DONATIONPOINT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GIVE_FOOD_TEST_DONATIONPOINT_ENTID"])
