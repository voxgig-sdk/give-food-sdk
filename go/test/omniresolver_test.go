...
import (
	...
	omni "github.com/voxgig-sdk/give-food-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GiveFoodError); is {
				out := map[string]any{
					"name":    "GiveFoodError",
...
func omniCtx(arg any, client *sdk.GiveFoodSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GiveFoodSDK); is {
			client = live
