...
func TestFoodbankEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		for _, _op := range []string{"list", "load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "foodbank." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GIVE_FOOD_TEST_FOODBANK_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		foodbankRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.foodbank", setup.data)))
...
func foodbankBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "foodbank", "FoodbankTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read foodbank test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse foodbank test data: " + err.Error())
	}
	...
	idmap := vs.Transform(
		[]any{"foodbank01", "foodbank02", "foodbank03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("GIVE_FOOD_TEST_FOODBANK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["GIVE_FOOD_TEST_FOODBANK_ENTID"])
