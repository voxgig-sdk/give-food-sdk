...
The Golang SDK for the GiveFood API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/give-food-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/give-food-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/give-food-sdk/go=../give-food-sdk/go
```
...
article, err := client.Article(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *GiveFoodSDK
```
...
All entities implement the `GiveFoodEntity` interface.
...
slice):
    ...
    article, err := client.Article(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // article is the returned record
...
```
github.com/voxgig-sdk/give-food-sdk/go/
├── give-food.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/give-food-sdk/go`) re-exports everything needed
