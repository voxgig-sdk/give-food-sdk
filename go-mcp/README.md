...
[MCP](https://modelcontextprotocol.io) server exposing the GiveFood SDK as
two agent tools — `give-food_list` and `give-food_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/give-food-mcp)
make build

# 2. Provide credentials via the environment
export GIVE_FOOD_APIKEY=sk_live_xxx
...
```jsonc
// give-food_list: first page of records
{ "entity": "article" }
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/give-food-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user give-food \
     -- "$PWD"/dist/*/give-food-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `give-food_list` and `give-food_load` tools now appear
   in new sessions. Ask the agent to *"list article using give-food"*
   and it calls `give-food_list` with `{"entity":"article"}`.
...
```sh
export GIVE_FOOD_APIKEY=sk_live_xxx            # API key
export GIVE_FOOD_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `give-food_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `give-food_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GIVE_FOOD_APIKEY` | API key sent with every request. |
| `GIVE_FOOD_BASE` | Optional override of the API base URL. |
