...
from givefood_sdk.givefood_types import (
    Donationpoint,
    DonationpointLoadMatch,
    DonationpointListMatch,
)
...
class DonationpointEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DonationpointEntity(self._client, opts)
    ...
    def data_get(self) -> Donationpoint:
    ...
    def match_get(self) -> Donationpoint:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Donationpoint:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Donationpoint().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Donationpoint]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Donationpoint().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
