...
The Python SDK for the GiveFood API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Article(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.GiveFood(None).load(
    {"id": "test01"}, None
)
