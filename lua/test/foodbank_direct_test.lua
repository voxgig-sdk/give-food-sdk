...
describe("FoodbankDirect", function()
  it("should direct-list-foodbank", function()
    local setup = foodbank_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-foodbank", setup.live and "live" or "unit")
  ...
  it("should direct-load-foodbank", function()
    local setup = foodbank_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-foodbank", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function foodbank_direct_setup(mockres)
