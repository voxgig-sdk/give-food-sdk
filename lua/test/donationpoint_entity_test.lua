...
describe("DonationpointEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Donationpoint(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Donationpoint(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GIVE_FOOD_TEST_DONATIONPOINT_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local donationpoint_ref01_data_dt0_load_result = helpers.to_map(type(donationpoint_ref01_data_dt0_loaded) == 'table' and donationpoint_ref01_data_dt0_loaded.data_get and donationpoint_ref01_data_dt0_loaded:data_get() or donationpoint_ref01_data_dt0_loaded)
    assert.is_not_nil(donationpoint_ref01_data_dt0_load_result)
    assert.are.equal(donationpoint_ref01_data_dt0_load_result["id"], donationpoint_ref01_data["id"])
...
function donationpoint_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GIVE_FOOD_TEST_DONATIONPOINT_ENTID")
