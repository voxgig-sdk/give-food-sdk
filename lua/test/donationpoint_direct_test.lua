...
describe("DonationpointDirect", function()
  it("should direct-list-donationpoint", function()
    local setup = donationpoint_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-donationpoint", setup.live and "live" or "unit")
  ...
  it("should direct-load-donationpoint", function()
    local setup = donationpoint_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-donationpoint", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function donationpoint_direct_setup(mockres)
