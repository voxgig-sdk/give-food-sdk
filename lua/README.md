...
The Lua SDK for the GiveFood API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Article(nil):list(nil, nil)
...
local result, err = client:GiveFood(nil):load(
  { id = "test01" }, nil
)
