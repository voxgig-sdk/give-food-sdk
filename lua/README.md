...
The Lua SDK for the GiveFood API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Article()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `needs` | `table` | Current needs status |
...
| `id` | `number` | Unique identifier for the item need record |
...
The main module (`give-food_sdk`) exports the SDK constructor
