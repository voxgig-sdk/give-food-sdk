...
local FoodbankEntity = {}
FoodbankEntity.__index = FoodbankEntity


function FoodbankEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, FoodbankEntity)
  self._name = "foodbank"
  self._client = client
...
function FoodbankEntity:get_name()
  return self._name
end


function FoodbankEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return FoodbankEntity.new(self._client, opts)
end
...
function FoodbankEntity:mark_deleted()
  self._deleted = true
end


function FoodbankEntity:deleted()
  return true == self._deleted
end


function FoodbankEntity:data_set(args)
  if args ~= nil then
...
function FoodbankEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function FoodbankEntity:match_set(args)
  if args ~= nil then
...
function FoodbankEntity:match_get()
...
function FoodbankEntity:stream(action, args, callopts)
...
function FoodbankEntity:_run_op(ctx, post_done)
  local utility = self._utility
