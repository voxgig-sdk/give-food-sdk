...
local DonationpointEntity = {}
DonationpointEntity.__index = DonationpointEntity


function DonationpointEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DonationpointEntity)
  self._name = "donationpoint"
  self._client = client
...
function DonationpointEntity:get_name()
  return self._name
end


function DonationpointEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DonationpointEntity.new(self._client, opts)
end
...
function DonationpointEntity:mark_deleted()
  self._deleted = true
end


function DonationpointEntity:deleted()
  return true == self._deleted
end


function DonationpointEntity:data_set(args)
  if args ~= nil then
...
function DonationpointEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DonationpointEntity:match_set(args)
  if args ~= nil then
...
function DonationpointEntity:match_get()
...
function DonationpointEntity:stream(action, args, callopts)
...
function DonationpointEntity:_run_op(ctx, post_done)
  local utility = self._utility
