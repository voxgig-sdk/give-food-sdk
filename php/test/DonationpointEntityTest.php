...
class DonationpointEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GiveFoodSDK::test(null, null);
        $ent = $testsdk->Donationpoint(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Donationpoint(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Donationpoint(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = donationpoint_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "donationpoint." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GIVE_FOOD_TEST_DONATIONPOINT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $donationpoint_ref01_data = null;
        if (count($donationpoint_ref01_data_raw) > 0) {
...
function donationpoint_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/donationpoint/DonationpointTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["donationpoint01", "donationpoint02", "donationpoint03"] as $k) {
    ...
    $entid_env_raw = getenv("GIVE_FOOD_TEST_DONATIONPOINT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["GIVE_FOOD_TEST_DONATIONPOINT_ENTID"]);
