...
{
    private static function utility(): GiveFoodUtility
    {
        return new GiveFoodUtility();
    }
    ...
    private static function ctx(array $over = []): GiveFoodContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GiveFoodContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GiveFoodOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GiveFoodError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GiveFoodOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GiveFoodMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GiveFoodOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GiveFoodMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GiveFoodOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GiveFoodMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GiveFoodSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GiveFoodMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GiveFoodResponse([]),
            'result' => new GiveFoodResult([]),
        ]);
        [, $err] = GiveFoodMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GiveFoodSpec([]),
            'response' => null,
            'result' => new GiveFoodResult([]),
        ]);
        [, $err] = GiveFoodMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GiveFoodSpec(['step' => 's']),
            'response' => new GiveFoodResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GiveFoodResult([]),
        ]);
        [, $err] = GiveFoodMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GiveFoodSpec(['step' => 's']),
            'response' => new GiveFoodResponse(self::resp(200, ['v' => 1])),
            'result' => new GiveFoodResult([]),
        ]);
        [, $err] = GiveFoodMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GiveFoodSpec(['step' => 's']),
            'response' => new GiveFoodResponse(self::resp(200, ['v' => 2])),
            'result' => new GiveFoodResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GiveFoodMakeResponse::call($ctx);
    ...
    {
        $preset = new GiveFoodResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GiveFoodSpec([]),
            'response' => new GiveFoodResponse([]),
            'result' => new GiveFoodResult([]),
        ...
        [$got, $err] = GiveFoodMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GiveFoodResult([])]);
        [, $err] = GiveFoodMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GiveFoodSpec([]), 'result' => null]);
        [, $err] = GiveFoodMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GiveFoodSpec(['step' => 's']),
            'result' => new GiveFoodResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GiveFoodOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GiveFoodMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GiveFoodSpec(['step' => 's']),
            'result' => new GiveFoodResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GiveFoodOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GiveFoodMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GiveFoodResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GiveFoodSpec([]),
            'result' => new GiveFoodResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GiveFoodMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GiveFoodMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GiveFoodError('boom', 'boom');
        $utility->fetcher = function (GiveFoodContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GiveFoodSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GiveFoodMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GiveFoodContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GiveFoodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GiveFoodResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GiveFoodContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GiveFoodSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GiveFoodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GiveFoodResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GiveFoodSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GiveFoodMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GiveFoodMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GiveFoodMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GiveFoodResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GiveFoodDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GiveFoodResult(['ok' => false])]);
        $this->expectException(GiveFoodError::class);
        GiveFoodDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GiveFoodResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GiveFoodMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GiveFoodMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GiveFoodBaseFeature();
        $b = new GiveFoodBaseFeature();
        GiveFoodFeatureAdd::call($ctx, $a);
        GiveFoodFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GiveFoodBaseFeature
    {
        $f = new GiveFoodBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GiveFoodFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GiveFoodContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GiveFoodSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GiveFoodPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GiveFoodResponse(['headers' => null]),
            'result' => new GiveFoodResult([]),
        ]);
        GiveFoodResultHeaders::call($ctx);
