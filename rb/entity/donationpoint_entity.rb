...
class DonationpointEntity
  ...
  def make
    opts = @_entopts.dup
    DonationpointEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GiveFoodHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GiveFoodHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.Donationpoint.load works with no args).
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GiveFoodHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DonationpointListMatch, Hash, nil] match filter (any subset of
  #   Donationpoint fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
