...
class FoodbankEntity
  ...
  def make
    opts = @_entopts.dup
    FoodbankEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Foodbank.load works with no args).
  ...
  #
  # @param reqmatch [FoodbankListMatch, Hash, nil] match filter (any subset of
  #   Foodbank fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
