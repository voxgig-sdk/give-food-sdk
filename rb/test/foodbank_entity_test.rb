...
class FoodbankEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = GiveFoodSDK.test(seed, nil)
    seen = base.Foodbank(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Foodbank(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = foodbank_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "foodbank." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set GIVE_FOOD_TEST_FOODBANK_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    foodbank_ref01_data = nil
    if foodbank_ref01_data_raw.length > 0
...
def foodbank_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "foodbank", "FoodbankTestData.json")
  ...
  entid_env_raw = ENV["GIVE_FOOD_TEST_FOODBANK_ENTID"]
  ...
  if env["GIVE_FOOD_TEST_LIVE"] == "TRUE"
    ...
    client = GiveFoodSDK.new(Helpers.to_map(merged_opts))
  end
