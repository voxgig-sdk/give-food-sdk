...
boru-driven command-line client **and** interactive REPL for the GiveFood
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/give-food-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/give-food-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/give-food-cli list article
   ```
...
Run with no arguments to open a REPL (prompt `give-food>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `article`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/give-food-cli`. |
...
(`./give-food-cli <expr>`) and interactively (the REPL), and expressions compose the
