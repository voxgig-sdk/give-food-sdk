...
module GiveFoodUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = GiveFoodResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GiveFoodResponse.new(fetched)
    else
