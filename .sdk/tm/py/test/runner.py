...
class GiveFoodTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GiveFoodTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GiveFoodTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GiveFoodTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = GiveFoodTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GiveFoodTestRunner.getenv(key)
        ...
        explain = GiveFoodTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if GiveFoodTestRunner._test_control is not None:
            return GiveFoodTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GiveFoodTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GiveFoodTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GiveFoodTestRunner.load_test_control()
...
def load_env_local():
    GiveFoodTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return GiveFoodTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GiveFoodTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GiveFoodTestRunner.load_test_control()


def live_client_options():
    return GiveFoodTestRunner.live_client_options()


def live_delay_ms():
    return GiveFoodTestRunner.live_delay_ms()
