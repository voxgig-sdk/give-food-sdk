...
class FtCtrl extends GiveFoodControl
{
...
class FtClient
{
    ...
    private GiveFoodUtility $utility;

    public function __construct(GiveFoodUtility $utility, array $options)
    {
    ...
    public function get_utility(): GiveFoodUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GiveFoodContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GiveFoodContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GiveFoodContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GiveFoodConfig::make_feature($name);
    ...
    private static function buildUrl(GiveFoodSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GiveFoodError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GiveFoodResponse($fetched);
        }
    ...
    private function fail(GiveFoodContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GiveFoodContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
