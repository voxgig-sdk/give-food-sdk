...
//     (\`new GiveFoodSDK(...)\` / \`GiveFoodSDK.test(...)\` become
//     \`GiveFoodSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"article":{"test01":{"id":"test01"}},"donationpoint":{"test01":{"id":"test01"}},"foodbank":{"test01":{"id":"test01"}},"item":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GiveFoodSDK(...)\` and any
// \`GiveFoodSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GiveFoodSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GiveFoodSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
