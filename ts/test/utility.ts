...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.GIVEFOOD_TEST_LIVE ||
    'TRUE' === process.env.GIVEFOOD_TEST_OVERRIDE
  ) {
  ...
  m.GIVEFOOD_TEST_EXPLAIN = process.env.GIVEFOOD_TEST_EXPLAIN || m.GIVEFOOD_TEST_EXPLAIN
