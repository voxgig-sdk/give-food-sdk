...
(0, node_test_1.describe)('DonationpointDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-donationpoint', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-donationpoint', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-donationpoint', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-donationpoint', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.GIVE_FOOD_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.GiveFoodSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['GIVE_FOOD_TEST_DONATIONPOINT_ENTID'];
    ...
    const client = new __1.GiveFoodSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=DonationpointDirect.test.js.map